...
# ==============================================================================
# Goal G-230 Autonomous Socratic Runner & Zero-Mock Verification Engine
# ==============================================================================
...
GOAL_ID="G-230"
GOAL_FILE="$REPO_ROOT/docs/07-backlog/goals/G-230-ai-model-fallback-router-and-price-recalculator.md"
SOCRATIC_ENGINE="$REPO_ROOT/scripts/agentic/g230-ai-model-fallback-pricing-5why-socratic-engine.mjs"
HARNESS_SCRIPT="$REPO_ROOT/scripts/harness/g230-ai-model-fallback-pricing-harness.mjs"
...
echo -e "${BOLD}${CYAN}    AI Model Fallback Router & Dynamic Token Price Re-Calculator${RESET}"
...
TREATISE_DOC="$REPO_ROOT/docs/06_raw/20260831_162000_g230_ai_model_fallback_pricing_5why_socratic_treatise.md"
...
# Step 5: Run G-230 Zero-Mock Production Test Harness & Rust Backend Tests
echo -e "${BOLD}${MAGENTA}[Step 5/5] Running Zero-Mock Production Test Harness & Rust Backend Tests for ${GOAL_ID}...${RESET}"
node "$HARNESS_SCRIPT"
echo -e "  Running cargo unit & integration tests for G-230..."
(cd "$REPO_ROOT/code" && cargo test -p metering-engine ai_fallback --quiet)
(cd "$REPO_ROOT/code" && cargo test -p payment-service --quiet)
