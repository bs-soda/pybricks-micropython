...
# ==============================================================================
# Goal G-250 Autonomous Socratic Runner & Zero-Mock Verification Engine
# ==============================================================================
...
GOAL_ID="G-250"
GOAL_FILE="$REPO_ROOT/docs/07-backlog/goals/G-250-multi-entity-consolidation-and-erp-sync.md"
SOCRATIC_ENGINE="$REPO_ROOT/scripts/agentic/g250-multi-entity-consolidation-5why-socratic-engine.mjs"
HARNESS_SCRIPT="$REPO_ROOT/scripts/harness/g250-multi-entity-consolidation-harness.mjs"
...
echo -e "${BOLD}${CYAN}    Multi-Entity Accounting Consolidation & ERP General Ledger Sync${RESET}"
...
TREATISE_DOC="$REPO_ROOT/docs/06_raw/20260831_103900_g250_multi_entity_consolidation_and_erp_sync_5why_socratic_treatise.md"
if [ ! -f "$TREATISE_DOC" ]; then
    echo -e "  ❌ Treatise doc not found: $TREATISE_DOC"
    exit 1
fi
INVARIANT_COUNT=$(grep -c "Non-Negotiable Invariant:" "$TREATISE_DOC" || true)
...
# Step 5: Run G-250 Zero-Mock Production Test Harness & Rust Backend Tests
echo -e "${BOLD}${MAGENTA}[Step 5/5] Running Zero-Mock Production Test Harness & Rust Backend Tests for ${GOAL_ID}...${RESET}"
node "$HARNESS_SCRIPT"
echo -e "  Running cargo unit tests for G-250..."
(cd "$REPO_ROOT/code" && cargo test -p accounting-sync -p accounting-service --bin accounting-service --lib --quiet)
