#!/usr/bin/env bash
# ==============================================================================
# Goal G-242 Runner: Multi-Tier Partner Rev-Share & Agency Affiliate Attribution Ledger
#
# Steps:
# 1. switch/checkout branch (feature/G-242)
# 2. clarify G-242 (verify spec, BDD scenarios & acceptance criteria)
...
# 5. run G-242 Zero-Mock Production Test Harness and backend verification
# ==============================================================================
...
GOAL_ID="G-242"
GOAL_TITLE="Multi-Tier Partner Rev-Share & Agency Affiliate Attribution Ledger"
SOCRATIC_SCRIPT="scripts/agentic/g242-partner-revshare-ledger-5why-socratic-engine.mjs"
HARNESS_SCRIPT="scripts/harness/g242-partner-revshare-ledger-harness.mjs"
SPEC_DOC="docs/07-backlog/goals/G-242-multi-tier-partner-rev-share-ledger.md"
RAW_DOC="docs/06_raw/20260831_103800_g242_multi_tier_partner_rev_share_ledger_5why_socratic_treatise.md"
...
if [ "$CURRENT_BRANCH" != "feature/G-242" ] && [ "$CURRENT_BRANCH" != "feature/epic-fim" ]; then
  echo -e "  Switching to branch ${BOLD}feature/G-242${RESET}..."
  git checkout -B feature/G-242
fi
...
# Step 5: Run G-242 Zero-Mock Production Test Harness & Rust Backend Tests
echo -e "${BOLD}${MAGENTA}[Step 5/5] Running Zero-Mock Production Test Harness & Rust Backend Tests for ${GOAL_ID}...${RESET}"
node "$HARNESS_SCRIPT"
echo -e "  Running cargo unit tests for G-242..."
(cd "$REPO_ROOT/code" && cargo test -p accounting-sync -p payment-service --lib --quiet)
