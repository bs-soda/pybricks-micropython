 ...
 */
static pbio_error_t pbdrv_display_virtual_process_thread(pbio_os_state_t *state,
                                                         void *context) {
  ...
  for (;;) {
    ...
    pbdrv_rproc_virtual_socket_send(display_image.pixels,
                                    sizeof(pbdrv_display_user_frame));
  }
...
void pbdrv_display_init(void) {
  ...
  pbio_busy_count_up();
  pbio_os_process_start(&pbdrv_display_virtual_process,
                        pbdrv_display_virtual_process_thread, NULL);
}

pbio_image_t *pbdrv_display_get_image(void) { return &display_image; }

uint8_t pbdrv_display_get_max_value(void) { return 3; }
