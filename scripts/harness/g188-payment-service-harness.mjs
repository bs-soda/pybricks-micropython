...
/**
 * scripts/harness/g188-payment-service-harness.mjs
 * 
 * Production Harness for Goal G-188:
 * Validates payment-service microservice structure, exports, zero-mock invariants, and cargo tests.
 */
...
console.log('\x1b[1m\x1b[36m║   🧪  GOAL G-188: PAYMENT-SERVICE ZERO-MOCK PRODUCTION TEST HARNESS          ║\x1b[0m');
console.log('\x1b[1m\x1b[36m╚══════════════════════════════════════════════════════════════════════════════╝\x1b[0m\n');
...
if (fs.existsSync(rootCargoToml)) {
  ...
  check(
    'code/Cargo.toml includes "apps/services/payment-service"',
    rootCargoContent.includes('apps/services/payment-service'),
...
check('All payment-service files zero-mock compliant', zeroMockPassed, 'Stubs detected in src/');

// 4. Run Native Rust Unit & Integration Tests
console.log('\n🦀 \x1b[1m4. Running Native Rust Tests (cargo test -p payment-service):\x1b[0m');
try {
  const output = execSync('cargo test -p payment-service', {
...
if (passedChecks === totalChecks) {
  console.log('\x1b[32m\x1b[1m🏆 G-188 PAYMENT-SERVICE HARNESS VERIFIED 100% GREEN!\x1b[0m\n');
  process.exit(0);
} else {
  console.error('\x1b[31m\x1b[1m⚠️ G-188 HARNESS CHECKS FAILED.\x1b[0m\n');
  process.exit(1);
}
