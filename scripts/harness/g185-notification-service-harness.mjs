...
/**
 * scripts/harness/g185-notification-service-harness.mjs
 * 
 * Production Harness for Goal G-185:
 * Validates notification-service microservice structure, exports, templates, zero-mock invariants, and cargo tests.
 */
...
const SERVICE_DIR = path.join(REPO_ROOT, 'code/apps/services/notification-service');

console.log('\x1b[1m\x1b[36m╔══════════════════════════════════════════════════════════════════════════════╗\x1b[0m');
console.log('\x1b[1m\x1b[36m║   🧪  GOAL G-185: NOTIFICATION-SERVICE ZERO-MOCK TEST HARNESS                ║\x1b[0m');
console.log('\x1b[1m\x1b[36m╚══════════════════════════════════════════════════════════════════════════════╝\x1b[0m\n');
...
if (fs.existsSync(rootCargoToml)) {
  ...
  check(
    'code/Cargo.toml includes "apps/services/notification-service"',
    rootCargoContent.includes('apps/services/notification-service'),
...
check('All notification-service files zero-mock compliant', zeroMockPassed, 'Stubs detected in src/');

// 4. Run Native Rust Unit & Integration Tests
console.log('\n🦀 \x1b[1m4. Running Native Rust Tests (cargo test -p notification-service):\x1b[0m');
try {
  const output = execSync('cargo test -p notification-service', {
...
if (passedChecks === totalChecks) {
  console.log('\x1b[32m\x1b[1m🏆 G-185 NOTIFICATION-SERVICE HARNESS VERIFIED 100% GREEN!\x1b[0m\n');
  process.exit(0);
} else {
  console.error('\x1b[31m\x1b[1m⚠️ G-185 HARNESS CHECKS FAILED.\x1b[0m\n');
  process.exit(1);
}
