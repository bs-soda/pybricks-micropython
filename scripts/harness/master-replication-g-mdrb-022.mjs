#!/usr/bin/env node
/**
 * Master Replication & Complete Release Gate: G-MDRB-022
 * 
...
console.log('🏛️ Master Replication & Complete Release Gate: G-MDRB-022');
console.log('='.repeat(80));
...
// -----------------------------------------------------------------------------
console.log('\n▶ Gate 2: Touch Map SHA-256 Integrity Verification...');
const goalCardRelPath = existsSync(resolve(ROOT, 'docs/07-backlog/goals/G-MDRB-022.md'))
  ? 'docs/07-backlog/goals/G-MDRB-022.md'
  : 'docs/07-backlog/goals/_archived/G-MDRB-022.md';
...
const touchMap = [
  ...
  'docs/02-product/acceptance/G-MDRB-022.md',
  goalCardRelPath
];
...
const contract = readFileSync(resolve(ROOT, 'docs/02-product/acceptance/G-MDRB-022.md'), 'utf8');

assertGate(4, 'Preemption transactional immunity specified across turn, pivot, and nav',
  goalCard.toLowerCase().includes('preemption') && contract.toLowerCase().includes('preemption'),
  'Goal and contract must specify preemption immunity');

assertGate(4, 'Invalid parameter rejections mandate ValueError without stopping active motion',
  contract.includes('ValueError') && contract.includes('no servo stop'),
  'Acceptance contract must require ValueError without servo stop');
...
console.log('\n▶ Gate 6: Socratic Agentic Loop (5 Branches x Level 5 Dialectic)...');
try {
  const socraticOut = execSync('node scripts/harness/socratic-agentic-loop-g-mdrb-022-harness.mjs', { cwd: ROOT, encoding: 'utf8' });
...
// -----------------------------------------------------------------------------
console.log('\n▶ Gate 7: Acceptance Criteria Traceability Matrix...');
assertGate(7, 'Acceptance Contract: AC-MDRB-022-1 (Active navigation immunity to invalid turn replacement)',
  contract.includes('Scenario 1') && contract.includes('turn_to_angle') && contract.includes('ValueError'),
  'Acceptance contract must include Scenario 1');

assertGate(7, 'Acceptance Contract: AC-MDRB-022-2 (Active turn immunity to invalid pivot replacement)',
  contract.includes('Scenario 2') && contract.includes('pivot_turn') && contract.includes('ValueError'),
  'Acceptance contract must include Scenario 2');

assertGate(7, 'Acceptance Contract: AC-MDRB-022-3 (Active trajectory immunity to empty replacement trajectory)',
  contract.includes('Scenario 3') && contract.includes('follow_trajectory') && contract.includes('ValueError'),
  'Acceptance contract must include Scenario 3');

assertGate(7, 'Acceptance Contract: AC-MDRB-022-4 (Valid replacement motion preemption cleanly cancels active motion)',
  contract.includes('Scenario 4') && contract.includes('cleanly cancelled'),
  'Acceptance contract must include Scenario 4');

assertGate(7, 'Acceptance Contract: AC-MDRB-022-5 (Zero servo stop or actuator glitch during invalid dispatch)',
  contract.includes('Scenario 5') || contract.includes('no servo stop') || contract.includes('without interruption'),
  'Acceptance contract must require zero servo stop during error');
...
if (totalFail > 0) {
  ...
  console.log('   Goal G-MDRB-022 specification is complete, robust, and verified.\n');
  process.exit(0);
}
