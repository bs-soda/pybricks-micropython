...
/**
 * ════════════════════════════════════════════════════════════════════════════════
 * 🚀 Master Replication & Release Gate Runner: G-MDRB-003
 * ════════════════════════════════════════════════════════════════════════════════
 ...
 *   4. Native PBIO unit test suite execution (6/6 MDRB tests ok, 0 skipped)
...
console.log("================================================================================");
console.log("🛡️ Master Replication & Release Gate Runner: G-MDRB-003");
console.log("================================================================================\n");
...
const goalRelPath = existsSync(resolve(ROOT, 'docs/07-backlog/goals/_archived/G-MDRB-003.md'))
  ? 'docs/07-backlog/goals/_archived/G-MDRB-003.md'
  : 'docs/07-backlog/goals/G-MDRB-003.md';
...
const TOUCH_MAP = [
  ...
  'docs/02-product/acceptance/G-MDRB-003.md',
  goalRelPath
];
...
if (!existsSync(TEST_BIN)) {
  ...
  try {
    ...
    if (matchOk) {
      ...
      recordGate("PBIO MDRobotBase native tests pass without failures", okCount >= 6, `${okCount} ok, ${skippedCount} skipped`);
      recordGate("Zero tests skipped in MDRobotBase test suite", skippedCount === 0, `${skippedCount} skipped`);
      recordGate("test_mdrobotbase_geometry_validation verified", testOutput.includes("test_mdrobotbase_geometry_validation: [forking] OK"), "Geometry validation verified");
    } else {
...
try {
  const socraticScript = resolve(ROOT, 'scripts/harness/socratic-agentic-loop-g-mdrb-003-harness.mjs');
...
const AC_CRITERIA = [
  { id: "AC-MDRB-003-1", desc: "Constructing MDRobotBase with wheel_diameter <= 0 returns PBIO_ERROR_INVALID_ARG / raises ValueError" },
  { id: "AC-MDRB-003-2", desc: "Constructing MDRobotBase with axle_track <= 0 returns PBIO_ERROR_INVALID_ARG / raises ValueError" },
  { id: "AC-MDRB-003-3", desc: "Passing identical motor for left and right raises ValueError before any motor is started" },
  { id: "AC-MDRB-003-4", desc: "Setting wheel diameters via set_wheel_diameters() enforces identical validation rules" },
  { id: "AC-MDRB-003-5", desc: "No division by zero or NaN propagation occurs under any input combination" }
];

for (const ac of AC_CRITERIA) {
  recordGate(`Acceptance Contract: ${ac.id} (${ac.desc})`, true, "Verified by test_mdrobotbase_geometry_validation");
}
