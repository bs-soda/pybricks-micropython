#!/usr/bin/env node
/**
 * Master Replication & Complete Release Gate: G-MDRB-029
 *
 ...
 * - Gate 6: Two-Point Sensor Calibration Invariant Oracle
...
console.log('🏛️ Master Replication & Complete Release Gate: G-MDRB-029');
console.log('='.repeat(80));
...
// -----------------------------------------------------------------------------
console.log('\n▶ Gate 2: Touch Map SHA-256 Integrity Verification...');
const goalCardRelPath = existsSync(resolve(ROOT, 'docs/07-backlog/goals/G-MDRB-029.md'))
  ? 'docs/07-backlog/goals/G-MDRB-029.md'
  : 'docs/07-backlog/goals/_archived/G-MDRB-029.md';
...
assertGate(6, 'G-MDRB-029 specifies dark offset subtraction formula', gContent.includes('R - R_0') || gContent.includes('R - R_{black}'));
assertGate(6, 'G-MDRB-029 specifies white gain normalization to [0.0, 1.0]', gContent.includes('0.0, 1.0'));
assertGate(6, 'G-MDRB-029 specifies dynamic range guard C_w - C_0 >= 5.0', gContent.includes('C_w - C_0 \\ge 5.0'));
...
console.log('\n▶ Gate 7: Socratic Agentic Loop & Acceptance Criteria Traceability Matrix...');
try {
  const socOut = execSync('node scripts/harness/socratic-agentic-loop-g-mdrb-029-harness.mjs', { cwd: ROOT, encoding: 'utf8' });
...
assertGate(7, 'G-MDRB-029 contains 100% template checklist passing', gContent.includes('- [x] Intent is WHAT/WHY only'));
assertGate(7, 'G-MDRB-029 targets 10.0/10 calibration score', gContent.includes('Expected score:** 10.0/10'));
...
if (failedGates === 0) {
  console.log('🏆 100% GREEN ATTESTATION: G-MDRB-029 Enterprise Release Gate Fully Satisfied!\n');
  process.exit(0);
} else {
